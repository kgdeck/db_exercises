...
# Voraussetzungen
Du verfügst über einen Zugang zu einem Datenbank-Server (Oracle oder Postgres), auf dem die Realisierung des Bike-Verleih-Szenarios erfolgreich installiert wurde. Falls dies noch nicht geschehen ist, kannst Du die die Inhalte der Dateien `create_tables_all.sql` und `insert_all.sql` über ein Client-Programm (Copy-and-Paste in den Eingabebereich) als Skript ausführen.
...
Welche SQL-Anweisungen liefern eine Liste aller Mitarbeitenden, wobei die Attribute `ID`, `NAME` und `SALARY` angezeigt werden? Die Ausgabereihenfolge soll einmal nach `SALARY` aufsteigend, einmal nach `SALARY` absteigend erfolgen. Was fällt bezüglich `NULL` bei `SALARY` auf?

::: {.content-hidden unless-meta="show_solutions"}
**Lösungen:**
```sql
SELECT ID, NAME, SALARY FROM SHOP
ORDER BY SALARY
...
Es fällt auf, dass die Datensätze mit `NULL` im Sortierkriterium (hier `SALARY`) im aufsteigenden Fall am Ende und im absteigenden Fall zu Beginn angezeigt werden. Dieses Verhalten gilt etwa bei Oracle- und PostgreSQL-Systemen; bei MySQL, MariaDB und MS-SQLServer erscheinen diese Werte immer am Ende.
:::
...
Hinweis: Sortiere die Mitarbeitenden entsprechend und limitiere (LIMIT) das Resultat. Beachte jedoch das in a) beschriebene Verhalten hinsichtlich `NULL`.   
...
```  
Anstelle der `WHERE`-Klausel könnte man auch `ORDER BY SALARY DESC NULLS LAST` verwenden. 
:::

Zusatzfrage (ausserhalb der bisher behandelten Themen): Wie könnte man das Resultat nach Salär aufsteigend sortiert ausgeben?  
Hinweis: Eine mögliche Lösung verwendet die vorherige Lösung als sogenannte FROM-Subquery.

::: {.content-hidden unless-meta="show_solutions"}
**Lösung:**
 
Das Ergebnis der ursprünglichen Lösung könnte man noch wie folgt umsortieren:
```sql  
...
Tipp:  Verwende `CURRENT_DATE` zusammen mit der Funktion `COALESCE()`. Der Ausdruck `COALESCE(A, B)` liefert den Wert von `A`, sofern dieser nicht `NULL` ist, sonst `B`.
